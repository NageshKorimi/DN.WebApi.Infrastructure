...
internal static class Startup
{
	internal static IServiceCollection AddSwaggerDocumentation(this IServiceCollection services, IConfiguration config)
	{
		...
		if (settings.Enable)
		{
			services.AddVersionedApiExplorer(o => { o.SubstituteApiVersionInUrl = true; o.DefaultApiVersion = new ApiVersion(1, 0); });
			...
			services.AddOpenApiDocument(document =>
			{
				document.PostProcess = doc =>
				{
					...
					if (config.GetValue<bool>("HangfireSettings:Enable"))
					{
						doc.ExternalDocumentation = new()
						{
							Description = "Hangfire Server Dashboard",
							Url = config.GetValue<string>("HangfireSettings:Route")
				...
				if (config.GetValue<bool>("SecuritySettings:EnableSecurity"))
				{
	...
	internal static IApplicationBuilder UseSwaggerDocumentation(this IApplicationBuilder app, IConfiguration config)
	{
		...
		if (settings.Enable)
		{
			...
			app.UseSwaggerUi3(options =>
			{
				...
				if (config.GetValue<bool>("SecuritySettings:EnableSecurity") && config["SecuritySettings:Provider"].Equals("AzureAd", StringComparison.OrdinalIgnoreCase))
				{
